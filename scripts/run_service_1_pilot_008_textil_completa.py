...
FIXTURE = ROOT / "prueba_excels" / "la_textil_cosida_srl_mar_abr_may_2026.xlsx"
SHEET_NAME = "ventas"
WORK_DIR = ROOT / ".tmp" / "service_1_pilot_008_textil_completa"
...
def _headers() -> list[str]:
    ...
    try:
        ...
        return [str(value).strip() for value in first_row if value is not None and str(value).strip()]
    finally:
        workbook.close()
...
def _owner_column_answers(headers: list[str]) -> dict[str, str]:
    return {
        header: f"La columna {header} pertenece al registro operativo de ventas de la textil."
        for header in headers
    }


def _canonical_semantic_answers(first_result: dict[str, object]) -> tuple[dict[str, str], dict[str, list[str]]]:
    ...
    for question in questions:
        ...
        canonical = [value for value in allowed if value not in {"OTHER", "IGNORE", "IGNORED_NOT_RELEVANT"}]
        if not canonical:
            raise RuntimeError(f"No canonical semantic option available for {column_name!r}.")
...
def main() -> int:
    ...
    observation = {
        "schema_version": "SERVICE_1_PILOT_008_TEXTIL_COMPLETA_OBSERVATION_V1",
        "cycle": "CYCLE_037_RUN_S1_PILOT_008_TEXTIL_COMPLETA",
        ...
        "first_pass": {
            "status": first["status"],
            "owner_questions_count": len(first["product_pipeline"]["owner_questions"]),
        ...
        "final_pass": {
            ...
            "semantic_bindings_confirmed": product.get("semantic_bindings_confirmed"),
            "tools_executed": product.get("tools_executed"),
            "executed_tool_refs": product.get("physical_run", {}).get("executed_tool_refs", []),
        ...
        "limits": [
            ...
            "Run validates the canonical product path on the ventas sheet of a multi-sheet textile workbook, not a full textile diagnosis.",
