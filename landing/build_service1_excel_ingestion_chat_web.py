...
function questionsFromProfile(profile){
  ...
  questions.push(makeQuestion({
    ...
    prompt_text:'¿Qué período o fecha cubre este archivo?'
  }));
  questions.push(makeQuestion({
    target_ref:`file:${profile.fileName}:owner_goal`,
    answer_type:'free_text',
    prompt_text:'¿Qué querés que PymIA te ayude a entender o revisar en este Excel?'
  }));
  return questions;
}
function makeQuestion(spec){
  const source='service_1_excel_ingestion_chat_web';
  const ref=buildStableRef(source, spec.target_ref);
  return {
    question_ref:ref,
...
function buildStableRef(source, targetRef){
  const s=slug(source), t=slug(targetRef);
  return t && t!=='unknown' ? `service_1:${s}:${t}` : `service_1:${s}:text_${shortHash(String(targetRef||''))}`;
}
...
function nextQuestion(){
  ...
  if(state.questionIndex >= state.questions.length){ els.chatState.textContent='Completo'; addMessage('pymia','Terminé las preguntas iniciales. Podés exportar la conversación como JSON gobernable.'); els.ownerInput.disabled=true; els.sendBtn.disabled=true; els.unknownBtn.disabled=true; return; }
  const q=state.questions[state.questionIndex]; els.chatState.textContent=`Pregunta ${state.questionIndex+1}/${state.questions.length}`; addMessage('pymia',q.prompt_text+'\n(Podés responder: '+q.allowed_owner_responses.join(' / ')+'.)');
}
function answerCurrent(unknown){
  const q=state.questions[state.questionIndex]; if(!q){ return; }
  const raw=unknown?'NO SÉ / requiere revisión humana':els.ownerInput.value.trim();
  if(!raw){ alert('Escribí una respuesta o usá No sé.'); return; }
  state.answers.push(buildOwnerAnswerRecord(q, raw));
  addMessage('owner',raw); els.ownerInput.value='';
  nextQuestion();
}
function buildOwnerAnswerRecord(question, rawOwnerAnswer){
  return {
    question_ref:question.question_ref,
    target_ref:question.target_ref,
    raw_owner_answer:rawOwnerAnswer,
...
function exportConversation(){
  const payload={marker:'PYMIA_SERVICE_1_EXCEL_INGESTION_CHAT_V1', created_at:new Date().toISOString(), runtime_authorized:false, reexecution_authorized:false, recalculation_authorized:false, production_allowed:false, final_diagnosis:false, final_accounting_result:false, file_profile:state.profile, questions:state.questions, answers:state.answers};
