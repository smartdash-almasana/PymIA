...
function buildProfile(){ const sheets=state.workbook.SheetNames.map(name=>{ const rows=state.sheets[name]||[]; const nonEmpty=rows.filter(r=>r.some(c=>String(c).trim()!=='')); const maxCols=Math.max(0,...rows.map(r=>r.length)); const headerRow=findHeaderRow(rows); const headers=headerRow>=0 ? rows[headerRow].map(v=>String(v).trim()).filter(Boolean) : []; const emptyHeaders=maxCols-headers.length; return {name, rows:nonEmpty.length, cols:maxCols, headerRow, headers, emptyHeaders, signals:detectSignals(name,headers)} }); return {fileName:state.file.name, fileSize:state.file.size, sheets}; }
...
function detectSignals(sheetName,headers){ const text=(sheetName+' '+headers.join(' ')).toLowerCase(); const signals=[]; const rules=[['ventas',['venta','ventas','factura','cliente','precio','importe']],['cobranzas',['cobro','cobranza','pagado','saldo','recibo']],['banco',['banco','movimiento','debito','credito','extracto','cuenta']],['stock',['stock','inventario','sku','producto','cantidad']],['costos',['costo','coste','proveedor','compra']],['precios',['precio','lista','tarifa']]]; rules.forEach(([label,words])=>{if(words.some(w=>text.includes(w))) signals.push(label)}); return signals; }
function buildQuestions(profile){ const qs=[]; qs.push(q('archivo_proposito',`Leí el archivo "${profile.fileName}". ¿Qué representa este archivo en tu operación?`, 'PymIA necesita ubicar el archivo antes de tratar columnas.')); qs.push(q('periodo', '¿Qué período cubre este archivo?', 'Ejemplo: semana, mes, trimestre, cierre, exportación puntual.')); qs.push(q('hoja_principal', `Detecté estas hojas: ${profile.sheets.map(s=>s.name).join(', ')}. ¿Cuál es la hoja principal para revisar primero?`, 'No asumo que la primera hoja sea la correcta.')); profile.sheets.forEach(sheet=>{ qs.push(q(`sentido_hoja_${safeId(sheet.name)}`, `En la hoja "${sheet.name}" veo ${sheet.rows} filas con contenido y ${sheet.cols} columnas. ¿Qué contiene esta hoja?`, 'Confirmación de sentido de hoja.')); if(sheet.headers.length){ qs.push(q(`columnas_clave_${safeId(sheet.name)}`, `En "${sheet.name}" aparecen columnas como: ${sheet.headers.slice(0,10).join(', ')}. ¿Cuáles son las 3 columnas más importantes?`, 'Confirmación semántica de columnas.')); } else { qs.push(q(`sin_encabezado_${safeId(sheet.name)}`, `No pude identificar encabezados claros en "${sheet.name}". ¿Dónde empiezan los datos y qué significa cada bloque?`, 'Bloqueo potencial por encabezados débiles.')); } if(sheet.signals.length){ qs.push(q(`foco_${safeId(sheet.name)}`, `Por nombres de hoja/columnas, esta hoja parece relacionada con: ${sheet.signals.join(', ')}. ¿Ese foco es correcto?`, 'PymIA propone hipótesis de archivo, no diagnóstico.')); } }); qs.push(q('objetivo_revision', '¿Qué querés que se revise primero con este archivo?', 'Ordenar, entender columnas, preparar conciliación, revisar stock, ventas/cobranzas, precios/costos.')); qs.push(q('dudas_columnas', '¿Qué columnas o valores te generan duda?', 'Ayuda a evitar interpretación automática equivocada.')); qs.push(q('evidencia_faltante', '¿Existe otro archivo que debería mirarse junto con este?', 'Ejemplo: banco, cobranzas, ventas, stock, lista de precios, export de sistema.')); return qs; }
