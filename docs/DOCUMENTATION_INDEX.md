...
| `docs/arquitectura/HERMES_SM2_VM_MCP_ROUNDTRIP_RESULT.md` | **VIGENTE** | Resultado operativo SM2-VM: roundtrip MCP fallido por stub no compatible JSON-RPC y riesgo confirmado de suplantación de PymIA | Hermes Orchestrator / Arquitectura Maestra | ADR-008 | — | Futuro runbook SM2 con MCP server real | VM `smartpyme-factory`, futuro `pymia/mcp/server.py` | **Preservar como evidencia. ADR-008 no puede aprobarse hasta repetir SM2 con MCP server real.** |
