...
class Service1OwnerDeliveryPackageFileV1(TypedDict):
    filename: str
    source_path: str
    package_path: str
    sha256: str
...
class Service1OwnerDeliveryPackageV1(TypedDict):
    schema_version: str
    service_name: str
    case_id: str
    case_name: str
    ...
    readme_path: str
    manifest_path: str
    files: list[Service1OwnerDeliveryPackageFileV1]
    file_count: int
    runtime_authorized: bool
    notes: list[str]


def build_service_1_owner_delivery_package_v1(
    *,
    ...
    if not case_id:
        raise ValueError("SERVICE_1_OWNER_DELIVERY_PACKAGE_V1 requires case_id.")
    if harness_run["runtime_authorized"]:
        raise ValueError("SERVICE_1_OWNER_DELIVERY_PACKAGE_V1 rejects runtime_authorized=True.")

    package_dir = package_root_path / f"{case_id}_delivery_package"
    package_dir.mkdir(exist_ok=True)

    source_paths = _collect_source_paths(harness_run)
    copied_files: list[Service1OwnerDeliveryPackageFileV1] = []
    ...
    return {
        ...
        "notes": [
            "Owner delivery package created from audited controlled delivery demo harness output.",
...
def _build_file_record(*, source_path: Path, package_path: Path) -> Service1OwnerDeliveryPackageFileV1:
