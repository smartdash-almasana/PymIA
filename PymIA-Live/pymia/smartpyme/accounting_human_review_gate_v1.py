...
def evaluate_accounting_human_review_gate_v1(*, gate_input: GateInput) -> GateResult:
    ...
    if gate_input.get("decision") == "REJECTED":
        return _build_result("REJECTED", False, "revise_accounting_sandbox_candidate", ["Human review rejected the candidate."])
    if gate_input.get("decision") == "APPROVED":
        return _build_result("PASS", True, "prepare_accounting_sandbox_contract", ["Human review passed for sandbox candidate only."])
    return _build_result("PENDING", False, "request_human_accounting_review_decision", ["Human review is pending."])
