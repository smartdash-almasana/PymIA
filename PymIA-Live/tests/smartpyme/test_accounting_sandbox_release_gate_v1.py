...
def test_pass_allows_only_sandbox_candidate_and_never_runtime() -> None:
    result = evaluate_accounting_sandbox_release_gate_v1(gate_input=_gate_input())
...
def test_pending_sandbox_release_decision_blocks_candidate() -> None:
    gate_input = _gate_input()
    gate_input["decision"] = "PENDING"

    result = evaluate_accounting_sandbox_release_gate_v1(gate_input=gate_input)  # type: ignore[arg-type]

    assert result["status"] == "PENDING"
    assert result["sandbox_candidate_allowed"] is False
    assert result["runtime_authorized"] is False
    assert result["next_allowed_action"] == "request_accounting_sandbox_release_decision"


def test_rejected_sandbox_release_decision_blocks_candidate() -> None:
    gate_input = _gate_input()
...
def test_module_has_no_io_or_runtime_dependencies() -> None:
    import pymia.smartpyme.accounting_sandbox_release_gate_v1 as module
