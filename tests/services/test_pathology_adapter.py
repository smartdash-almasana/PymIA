...
def test_adapter_converts_active_pathology_to_finding_record():
    pathology = PathologyFinding(
        ...
        suggested_action="Revisar costos o precios.",
        source_refs=["ventas:1", "costos:1"],
        explanation="Margen negativo.",
        metadata={"measured_difference": {"margen_bruto": -0.2}},
    )

    finding = pathology_finding_to_finding_record(pathology)

    assert finding.entity == "pyme_A"
    assert finding.finding_type == "margen_bruto_negativo"
    assert finding.evidence_used == ["ventas:1", "costos:1"]
    assert finding.compared_sources == ["ventas:1", "costos:1"]
    assert finding.measured_difference == {"margen_bruto": -0.2}
