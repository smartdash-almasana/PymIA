...
def test_excel_fixture_executes_through_binder_and_core() -> None:
    ...
    assert result.formula_results[0].blocking_reason == "MISSING_INPUTS: sale_price,costs,taxes"
    assert result.formula_results[1].blocking_reason == "MISSING_INPUTS: sold_amount,collected_amount"
    assert result.formula_results[2].blocking_reason == "MISSING_INPUTS: cost_of_goods_sold,average_stock"
