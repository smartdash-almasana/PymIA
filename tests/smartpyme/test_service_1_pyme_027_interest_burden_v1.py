...
def _plan() -> dict[str, object]:
    return {
        ...
        "formula_id": "PYME_027_interest_burden_ratio",
...
def test_pyme_027_registry_contract_is_atomic_and_explicit() -> None:
    ...
    assert definition.formula_ref == "PYME_027_interest_burden_ratio"
...
def test_product_root_executes_pyme_027_once(monkeypatch, tmp_path) -> None:
    confirmed = {
    ...
    assert calls == ["interest_burden_ratio"]
...
def test_product_root_keeps_pyme_027_delivery_blocked(monkeypatch, tmp_path) -> None:
    confirmed = {
        "status": product.STATUS_CONFIRMED_BINDINGS,
        "schema_version": "TEST",
        "service_name": "SERVICE_1",
    ...
    assert result["blocked_reason"] == "PYME_027_DELIVERY_NOT_AUTHORIZED"
