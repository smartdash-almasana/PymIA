...
def test_controlled_delivery_demo_sample_case_creates_delivery_folder(tmp_path: Path) -> None:
    case = build_service_1_controlled_delivery_demo_sample_case_v1()

    result = run_service_1_controlled_delivery_demo_harness_v1(case=case, output_root=tmp_path)
...
def test_controlled_delivery_demo_runs_pipeline_and_generates_five_xlsx_files(tmp_path: Path) -> None:
    result = run_service_1_controlled_delivery_demo_harness_v1(
        case=build_service_1_controlled_delivery_demo_sample_case_v1(),
...
def test_controlled_delivery_demo_outputs_are_readable(tmp_path: Path) -> None:
    result = run_service_1_controlled_delivery_demo_harness_v1(
        case=build_service_1_controlled_delivery_demo_sample_case_v1(),
        output_root=tmp_path,
    )
...
def test_controlled_delivery_demo_writes_summary_and_delivery_report(tmp_path: Path) -> None:
    result = run_service_1_controlled_delivery_demo_harness_v1(
        case=build_service_1_controlled_delivery_demo_sample_case_v1(),
        output_root=tmp_path,
    )
    ...
    report_path = Path(result["delivery_report_path"])
...
def test_controlled_delivery_demo_summary_contains_owner_relevant_limits(tmp_path: Path) -> None:
    result = run_service_1_controlled_delivery_demo_harness_v1(
        case=build_service_1_controlled_delivery_demo_sample_case_v1(),
        output_root=tmp_path,
    )
...
def test_controlled_delivery_demo_preserves_custom_case_order(tmp_path: Path) -> None:
    case = {
        "case_id": "custom-case",
        ...
        "delivery_notes": ["custom note"],
    }

    result = run_service_1_controlled_delivery_demo_harness_v1(case=case, output_root=tmp_path)  # type: ignore[arg-type]
...
def test_controlled_delivery_demo_sanitizes_case_id_for_folder(tmp_path: Path) -> None:
    case = build_service_1_controlled_delivery_demo_sample_case_v1()
...
def test_controlled_delivery_demo_rejects_missing_output_root() -> None:
    with pytest.raises(FileNotFoundError, match="Output root does not exist"):
        run_service_1_controlled_delivery_demo_harness_v1(
            case=build_service_1_controlled_delivery_demo_sample_case_v1(),
            output_root="/nonexistent/controlled/delivery/demo/root",
        )


def test_controlled_delivery_demo_rejects_empty_case_id(tmp_path: Path) -> None:
    case = build_service_1_controlled_delivery_demo_sample_case_v1()
    case["case_id"] = "///"

    with pytest.raises(ValueError, match="requires a non-empty case_id"):
        run_service_1_controlled_delivery_demo_harness_v1(case=case, output_root=tmp_path)


def test_controlled_delivery_demo_rejects_empty_tool_requests(tmp_path: Path) -> None:
    case = build_service_1_controlled_delivery_demo_sample_case_v1()
    case["tool_requests"] = []
...
def test_controlled_delivery_demo_does_not_import_first_aid_tools_directly() -> None:
    import pymia.smartpyme.service_1_controlled_delivery_demo_harness_v1 as module
...
def test_controlled_delivery_demo_does_not_depend_on_forbidden_product_layers() -> None:
    import pymia.smartpyme.service_1_controlled_delivery_demo_harness_v1 as module
