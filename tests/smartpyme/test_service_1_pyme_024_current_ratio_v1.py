...
def test_product_root_executes_pyme_024_once(monkeypatch, tmp_path) -> None:
    confirmed = {
...
def test_product_root_keeps_pyme_024_delivery_blocked(monkeypatch, tmp_path) -> None:
    confirmed = {
        "status": product.STATUS_CONFIRMED_BINDINGS,
        "schema_version": "TEST",
        "service_name": "SERVICE_1",
    ...
    assert result["blocked_reason"] == "PYME_024_DELIVERY_NOT_AUTHORIZED"
    assert result["bounded_finding_generated"] is True
    assert result["delivery_generated"] is False
    assert result["delivery_authorized"] is False
