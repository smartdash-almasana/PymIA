...
def test_owner_delivery_package_creates_presentable_folder(tmp_path: Path) -> None:
...
def test_owner_delivery_package_inventory(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
    package_root = tmp_path / "packages"
    package_root.mkdir()
...
def test_owner_delivery_package_manifest_matches_artifacts(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
    package_root = tmp_path / "packages"
    package_root.mkdir()
...
def test_owner_delivery_package_files_include_hashes_and_sizes(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
    package_root = tmp_path / "packages"
    package_root.mkdir()
...
def test_owner_delivery_package_readme_is_client_and_delivery_readable(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
    package_root = tmp_path / "packages"
    package_root.mkdir()
...
def test_owner_delivery_package_copied_xlsx_files_are_readable(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
    package_root = tmp_path / "packages"
    package_root.mkdir()
...
def test_owner_delivery_package_rejects_missing_package_root(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
...
def test_owner_delivery_package_rejects_runtime_authorized_true(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
...
def test_owner_delivery_package_rejects_missing_artifact(tmp_path: Path) -> None:
    harness_run = _build_harness_run(tmp_path)
