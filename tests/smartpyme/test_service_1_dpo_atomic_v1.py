...
_COMPUTATION_PLAN_SCHEMA = "SERVICE_1_COMPUTATION_PLAN_V1"
...
def _plan(
    ...
    **overrides: object,
    ...
    base = {
        "schema_version": _COMPUTATION_PLAN_SCHEMA,
...
def test_dpo_below_period() -> None:
    ...
    result = execute_generic_capability_v1(capability_ref="dpo", computation_plan=_plan(), normalized_tables=tables, column_refs=refs)
...
def test_dpo_blocks_absent_flag() -> None:
    ...
    result = execute_generic_capability_v1(capability_ref="dpo", computation_plan=plan, normalized_tables=tables, column_refs=refs)
    assert result["status"] == STATUS_BLOCKED
...
def test_root_executes_dpo_once(monkeypatch, tmp_path) -> None:
    ...
    def tracking(*, capability_ref, computation_plan, normalized_tables, column_refs):
        ...
        return real_engine(capability_ref=capability_ref, computation_plan=computation_plan, normalized_tables=normalized_tables, column_refs=column_refs)
